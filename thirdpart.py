...
output1=np.subtract(data_Africa, pseudo(data_Africa))
plt.plot(output1) #plot the filtered signal as a graph
plt.title("Signal of Africa.wav") # add title to the graph
plt.show()

#write the output data into a file by adding BSP to its name
scipy.io.wavfile.write("AfricaBSP.wav", 44100, np.array(output1).astype(np.int16))

temp=pseudo(data_Winner_T_All)

result = tuple(map(lambda a, b: a - b, data_Winner_T_All, temp))
plt.plot(result) #plot the filtered signal as a graph
plt.title("Signal of WinnerTakesAll.wav") # add title to the graph
plt.show()

#write the output data into a file by adding BSP to its name
scipy.io.wavfile.write("WinnerTakesAllBSP.wav", 44100, np.array(output2).astype(np.int16))
