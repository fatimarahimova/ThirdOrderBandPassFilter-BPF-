...
samplerate, data_Africa = scipy.io.wavfile.read('Africa.wav')
samplerate, data_Winner_T_All = scipy.io.wavfile.read('WinnerTakesAll.wav')
...
output1=pseudo(data_Africa) #sending Africa.wav to the pseudo code
plt.plot(output1) #plot the filtered signal as a graph
plt.title("Signal of Africa.wav") # add title to the graph
plt.show()

#write the output data into a file by adding BPF to its name
scipy.io.wavfile.write("AfricaBPF.wav", 44100, np.array(output1).astype(np.int16))



output2=pseudo(data_Winner_T_All) #sending WinnerTakesAll.wav to the pseudo code
plt.plot(output2) #plot the filtered signal as a graph
plt.title("Signal of WinnerTakesAll.wav") # add title to the graph
plt.show()
#write the output data into a file by adding BPF to its name
scipy.io.wavfile.write("WinnerTakesAllBPF.wav", 44100, np.array(output2).astype(np.int16))
